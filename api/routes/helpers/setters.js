  // returns string if successful, error if not
async function saveCharacter(db, character, account, ladder_id) {
  console.log("checking for account...");
  const account_exists = await db
    .query(`SELECT id, name FROM accounts WHERE name=$1;`, [account])
    .then((result) => {
  ...
  if (account_exists.length > 0) {
    ...
    account_db = await db
      .query(`INSERT INTO accounts(name, twitch) VALUES($1, $2) RETURNING *;`, [
        account,
        twitch,
      ])
      .then((res) => {
...
async function saveAccount(db, accountName) {
  const account_exists = await db
    .query(`SELECT id, LOWER(name) FROM accounts WHERE LOWER(name)=$1;`, [
  ...
  if (account_exists.length > 0) {
    ...
    return await db
      .query(`INSERT INTO accounts(name) VALUES($1) RETURNING *;`, [
        accountName.toLowerCase(),
      ])
      .then((res) => {
...
async function saveLadder(db, name, ladder) {
  return new Promise(function (resolve, reject) {
    db.query(
      `INSERT INTO ladders(name, last_requested, rankings) VALUES($1, CURRENT_TIMESTAMP, $2) RETURNING *;`,
      [name, ladder]
    )
      .then((res) => {
