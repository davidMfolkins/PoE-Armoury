  ...
  if (filteredData) {
    rows = filteredData.slice(0, visible).map((entry) => {
      ...
      <td className="col-2">{entry.account.twitch && <a href={`https://twitch.tv/${entry.account.twitch.name}`} target="_blank" rel="noreferrer">{entry.account.twitch.name}</a>}</td>
          <td className="col-1"><LikeButton characterid={entry.character.id} favourites={props.favourites} character={entry.character} removeFavourite={props.removeFavourite} addFavourite={props.addFavourite} size="1.5em"/></td>
        </tr>
      )
    })
  }
